...
- [x] **US-T-01 + `cairn dev` subgroup:** `cairn dev serve` (detached HTTP MCP server with `start_new_session=True`, auto-port, XDG-cache state file), `cairn dev stop --pid|--all`, `cairn dev list`, `cairn dev scaffold-fixture {coral-bleach,lit-monitor,shared-physics-paper}`. Used by the multi-user/multi-cairn methodology at `tests/agent_smoke/multi-user-multi-cairn/` so sub-agents don't reinvent setup each run. Fixture data lives at `src/cairn/dev/fixtures_data.py` and mirrors `tests/agent_smoke/multi-user-multi-cairn/fixtures/README.md`.
