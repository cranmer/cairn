...
**MCP-first.** The primary integration is an MCP server (`cairn mcp`) that exposes ~28 read and write tools to any Claude Code session. One server serves all of a user's cairns (per [ADR-0010](docs/decisions/0010-single-mcp-server-multiple-cairns.md)); each MCP tool accepts a `cairn` parameter naming the target, defaulting to the only registered one when there's just one. Wire it up once with `claude mcp add --scope user cairn -- cairn mcp` and every session anywhere has access to the cairn without `cd` or per-session bootstrap. The same server also runs over HTTP for group / multi-machine setups — see [`docs/remote-mcp.md`](docs/remote-mcp.md).
...
| [docs/remote-mcp.md](docs/remote-mcp.md) | Operator + client guide for running `cairn mcp` over HTTP — Docker, reverse-proxy / TLS / auth pattern, remote pairing with `cairn link --endpoint` |
...
| Command | Purpose |
|---------|---------|
| `cairn mcp` | Run the MCP server. Defaults to stdio (configured via `claude mcp add --scope user cairn -- cairn mcp`); `--transport streamable-http` runs it over HTTP for group / multi-machine setups — see [`docs/remote-mcp.md`](docs/remote-mcp.md) |
...
| `cairn link [<project-repo>]` | Write a `cairn.toml` at a project repo root pairing it with a cairn. `--name` for a locally-registered cairn; `--endpoint <url> --name <handle>` for a remote-MCP cairn ([docs/remote-mcp.md](docs/remote-mcp.md)) |
