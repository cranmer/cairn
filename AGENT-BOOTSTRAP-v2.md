...
4. Decide: **start from scratch** (no source repo to bootstrap from) or **bootstrap from an existing repo** (most common — months/years of history to seed from). Also confirm whether this cairn is for the user alone or for a group sharing it over the network — if the latter, hand off (see Step 4).
5. Scaffold and register the cairn.
6. Register the user as the first collaborator.
...
> *"First: two ways to start. (A) **Start from scratch** — a brand-new cairn with no existing project repo to seed from. (B) **Bootstrap from an existing repo** — you have a project repo with months or years of history (README, ADRs, PR descriptions, contributor list) and want the cairn to pick that up as backdated decisions and findings before live capture begins. Which fits?"*

> *"Second: is this cairn just for you, or will multiple collaborators on different machines reach it over a network?"*

If the user says **"multiple collaborators / shared / network"**, this script is the wrong one for that flow. Tell them so and point them at `docs/group-deployment.md` (which covers running cairn as a long-lived HTTP server, credentials, and how each collaborator pairs with it). Then **stop** — let them decide whether to continue with a personal cairn here or switch to the group setup.

If they confirm **just for me, this machine**, continue with the rest of this script. Collect:
...
- Do not start a long-lived HTTP server on the user's behalf. If they want a group-shared cairn, hand off to `docs/group-deployment.md` instead of improvising.
